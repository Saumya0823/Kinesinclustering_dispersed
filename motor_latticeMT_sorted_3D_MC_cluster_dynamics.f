...
c        open(unit=10,file="Torque_diff_N15_D001.dat")
c        open(unit=11,file="angle_diff_N15_D001.dat")
c        open(unit=22,file="Rate_latticeMT_N15_D001.dat")
c        open(unit=23,file="RLV_latticeMT_N15_D001.dat")
	open(unit=20, file="motor_tails_motran_D1_N5_F0.dat")
        open(unit=21, file="time_points_motran_D1_N5_F0.dat")
...
c        imtd= 30
        variate= del_t/gamma
        vari2= del_t/gamnew
	vmot= (1/beadRadius)							!motor diffusion constant
...
c	     if(counter==0)then
c	      sum_x= sum_x + copy_beadmx(i)
c	      sum_y= sum_y + copy_beadmy(i)
c	      sum_z= sum_z + copy_beadmz(i)
c	      if (i==N)then
c	      print*, "here"
c              sum_x= sum_x/N; sum_x= sum_x/N; sum_x= sum_x/N
c	      j=i
c	      do j=1,N
c	      SD_x= sqrt((copy_beadmx(j)-sum_x)**2/N)
c              SD_y= sqrt((copy_beadmy(j)-sum_y)**2/N)
c              SD_z= sqrt((copy_beadmy(j)-sum_z)**2/N)
c              write(20,*)sigconf,time,j,-F_ext,sum_x,sum_y,sum_z,
c     1		SD_x, SD_y, SD_z
c	      end do
c	      end if
c	     end if
...
120        sm_area(itcount)= sm_area(itcount)+ total_area
