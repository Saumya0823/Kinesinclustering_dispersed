	...
	dimension thetamot(N),phimot(N),range(N),
     ...
     1          gamma= 8*pi*viscosity*Rcube, del_t=1e-5, maxconf=1000)
       ...
       open(unit=10,file="Torque_N10.dat")
       open(unit=11,file="angle_N10.dat")
       open(unit=22,file="Rate_N10.dat")
       open(unit=23,file="RLV_N10.dat")
c        open(unit=20, file="motor_tails_SD_rigid_N4.dat")
...
c            end do
c	    torq_sum= torq_sum + sum_torq(1)+ tau_ran(1)
c            torq_sum1= torq_sum1 + sum_torq(2)+ tau_ran(2)
c            torq_sum2= torq_sum2 + sum_torq(3)+ tau_ran(3)
c
c3           r1= 2*rand()-1
c            r2= 2*rand()-1
c            gauss= r1**2 + r2**2
...
c               F_mot(i)= (dup_stalk(i)-Lo)*springk                            ! magnitude of force exerted by attached and extended motors
            ...
            torq_sum= torq_sum + sum_torq(1)						!+ tau_ran(1)
            torq_sum1= torq_sum1 + sum_torq(2)						!+ tau_ran(2)
            torq_sum2= torq_sum2 + sum_torq(3)						!+ tau_ran(3)
...
c            itime= time*1e5
c            counter= mod(itime,20000)
c            if(counter==0)then
c            sum_x=0; sum_y=0; sum_z=0
c            do i=1,N
c             do j=1,N
c               conss= (copy_beadmx(i)-copy_beadmx(j))**2+
c     1          (copy_beadmy(i)-copy_beadmy(j))**2+
c     1          (copy_beadmz(i)-copy_beadmz(j))**2
c                sum_x= sum_x + sqrt(conss)
c              sum_x= sum_x + abs(copy_beadmx(i)-copy_beadmx(j))
c               sum_y= sum_x + abs(copy_beadmy(i)-copy_beadmy(j))
c               sum_z= sum_x + abs(copy_beadmz(i)-copy_beadmz(j))
c             end do
c             if(i==N)then
c                print*, "here"
c               sum_x= (sum_x*(N-2))/N
c              sum_y= (sum_y*(N-2))/N
c               sum_z= (sum_z*(N-2))/N
c              write(20,*)sigconf,time,-F_ext,sum_x                                      !,sum_y,sum_z
c             end if
c            end do
c            end if
...
c        write(11,*) "avg angular displacement at F=0 for 1000 conf="
        write(11,*) iF,N,imtd,beadRadius,avg_angle,avg_angle1,avg_angle2
         ...
         print*, -F_ext, -Favg, "detach= ",detavg," attach= ",attavg,
     1		" step= ",
