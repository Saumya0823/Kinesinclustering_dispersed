...
c	open(unit=21,file="Rate_rand_diff_V_N4_D1.dat")
c        open(unit=23,file="V_rand_diff_N4_D1.dat")
        open(unit=20, file="V_confare_rand_D1_N10.dat")
        open(unit=22, file="V_avgarea_rand_D1_N10.dat")
        open(unit=24, file="V_clus_rate_motran_D1_N10.dat")
...
c             end if
c             if((copy_headmx(i)-copy_beadmx(i)).ge.0)then
c                  Fall_mot(i)= -Fall_mot(i)
c             end if
c            end do
c
c          do i=1,N
...
c           if(con12==0)icont= int(itime/20000)
	   ...
	   if(icount==0 .and. itime .lt. 1.1e5)then
		go to 300
	   end if
           if(itime== 1.1e5)then
                if(icount==0)then
...
c          sum_torq(1)=0; sum_torq(2)=0; sum_torq(3)=0
c          do i=1,3
c                angle(i)= (sum_torq(i)*variate)+ tau_ran(i)
c               write(10,*) "angle=",angle(i)
c           end do
c
...
c            write(10,*)headmotx(i),headmoty(i),headmotz(i)
     ...
     1          (copy_beadmz(i)-copy_beadmz(j))**2
             ...
             write(20,*)-F_ext,sigconf,time,i,copy_beadmx(i),
...
c         write(21,*) iF,N,imtd,beadRadius,detavg,det_SD,det_SEM,
c     1          attavg,att_SD,att_SEM,stpavg,step_SD,step_SEM
c
c         write(23,*)iF,N,imtd,beadRadius,avgvel,vel_SD,vel_SEM,
c     1		avgact,active_SD,active_SEM
c
